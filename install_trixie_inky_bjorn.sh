...
if [[ "$1" == "--help" ]]; then
    echo "Usage: sudo ./install_trixie_inky_bjorn.sh"
...
install_dependencies() {
    ...
    ln -s /usr/lib/path/to/libopenblas.so.0 /usr/lib/libatlas.so.3
    ln -s /usr/lib/path/to/libopenblas.so.0 /usr/lib/libcblas.so.3
...
setup_bjorn() {
    ...
    if [ -d "Bjorn" ]; then
        ...
        git clone https://github.com/akrawczyk/Bjorn.git
